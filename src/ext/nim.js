...
export default {
  ...
  onCustomSysMsg(msg) {
    ...
    if (msg.type == "custom" && msg.scene == "p2p") {
      ...
      if (content.type == 10) {
        ...
        // 停止成员的屏幕共享，如果那个成员是当前的共享方
        const tryStopMemberScreenSharing = () => {
          console.log('取消对方的屏幕共享')
          let member = ChatroomState.members.find(member => member.account === msg.from)
          let chatroomCustom = ChatroomState.custom;
          if (member && (member.account == chatroomCustom.shareID)) {

            let obj ={...ChatroomState.custom};
            Object.assign(obj,{
              shareID: '', //Storage.get("teacherAccount"),
              showType: 1,
            })

            EXT_NIM.updateChatroom(obj)

            console.log(
              "**** 成员关闭屏幕共享， 主持人收到点对点后， 更新房间信息。",
            );
        ...
        switch (content.data.command) {
          ...
          case 12:
            ...
            Alert.open({
              title: "提示",
              msg:
                '<div class="u-end-interaction"><i class="u-icon-tip"></i>你被创建人取消互动</div>',
              isHtml: true,
              btns: [
                {
                  label: "确定",
          ...
          case 13: {
            ...
            // 尝试关闭非老师的屏幕共享
            tryStopMemberScreenSharing()
            break;
          }
          case 14: {

            console.log("互动者分享屏幕", msg);
            let chatroomCustom = ChatroomState.custom;
            if (chatroomCustom.shareID === "") {
              ...
              if (member && member.status === 1) {
                ...
                console.log(
                  "**** 成员开启屏幕共享， 主持人收到点对点后， 更新房间信息。"
                );
              }
            } else {
              console.warn('当前已有成员在共享屏幕: ', chatroomCustom)
            }

            break;
          }
          case 15: {
            console.log("收到取消屏幕共享请求", msg);
  ...
  sendCustomMsg(to, data) {
    const nim = NimState.nim;
      console.info("20200225 ext/nim sendCustomMsg");
    return new Promise((resolve, reject) => {
      nim.sendCustomMsg({
        scene: "p2p",
        to: to,
  ...
  sendCustomSysMsg(to, data) {
    const nim = NimState.nim;
      console.info("20200225 ext/nim sendCustomSysMsg");
    return new Promise((resolve, reject) => {
