...
export default {
        ...
        switch (attach.type) {
          ...
          case "memberEnter":
            ...
            if (!onlyFormatMsg) {
              ...
              if (msg.from == teacherAccount) {
                ...
                let showType = ChatroomState.custom.showType;
                if (shareID == account && shareID !== teacherAccount) {
                  EXT_NIM.sendCustomSysMsg(teacherAccount, {
                    room_id: ChatroomState.currChatroomId,
                    command: 15
                  }).then(() => {
                    console.log('通知老师端取消自己的屏幕共享状态成功！')
                  }).catch(e => {
                    console.error('### 告诉老师端关闭自己的共享成功')
                  })
                }
              }
              //老师端学生上线
          ...
          case "memberExit":
            if (!onlyFormatMsg) {
              ...
              if (Storage.get("isTeacher") == 1) {
                // 教师下收到成员退出，如果是屏幕共享方，需要断开他的屏幕共享
                NetcallAction.setVideoIsFullScreen(false);

                let shareID = ChatroomState.custom.shareID;
                if (shareID !== "" && shareID == msg.from) {
          ...
          case "updateChatroom":
            if (!onlyFormatMsg) {
              ...
              if (!('shareID' in custom)) {// ios不发这个，兼容
                custom.shareID = "";
              }
              console.log("custom: ", custom);

              let prevShareID = ChatroomState.custom.shareID;
              custom && ChatroomAction.mergeCustom(custom)

              let remoteIsScreenSharing = custom.showType == 0 && custom.shareID !== "";
              //设置远端是否正在屏幕共享
              NetcallAction.setHasShareScreen(remoteIsScreenSharing);
              NetcallAction.setRemoteIsScreenSharing(remoteIsScreenSharing);

              //关闭后处理（重新设置对应老师的画面节点）
              if (remoteIsScreenSharing === false) {
                console.log(" 重新设置分享者画面...");
                NetcallAction.setVideoIsFullScreen(false);
                const teacherAccount = Storage.get("teacherAccount");
                ...
                if (prevShareID !== '') {
                  ...
                  // EXT_NETCALL.stopRemoteStream(prevShareID)
                    console.info("20200221chatroom348")
                  EXT_NETCALL.startRemoteStream(
                    prevShareID,
                    NetcallState.doms[index]
                  );
                  if(index == 0) {
                      EXT_NETCALL.setVideoViewRemoteSize(prevShareID, 280, 208);
                  }
                  else {
                      EXT_NETCALL.setVideoViewRemoteSize(prevShareID);
                  }
                }
                if (NetcallState.shareStarted) {
  ...
  create(roomName) {
    ...
    const param = {
      url: env.url + "/api/netease/chatroom/create",
      type: "POST",
      data: {
    ...
    return Ajax(param)
      .then(data => {
        if (data.code === 1) {
          const id = data.data.chatroom.roomid;
  ...
  // 发送聊天室 相关信息
  sendChatRoomInfo(data) {
    const param = {
      url: "http://192.168.198.161:8762" + "/neteaseChatroom/saveNeteaseChatroom",
      type: "POST",
      data: data
    };
    return Ajax(param)
      .then(data => {
        if (data.res === 200) {
          ...
          return Promise.reject("发送聊天室信息失败");
        }
      })
      .catch(data => {
        console.error("发送聊天室信息失败");
        return Promise.reject(data);
      });
  },
    ...
    return new Promise((resolve, reject) => {
      window.chatroom = window.Chatroom.getInstance({
        ...
        onconnect: function (data) {
          console.log("聊天室连接已建立...", data);

          // debugger
          let custom = {}
          try {
            custom = data.chatroom.custom;
        ...
        onerror: function (error, obj) {
          if (error) {
            console.error("chatroom::onerror", error);
            ...
            switch (error.code) {
              ...
              case "kicked":
                if (error.reason === "managerKick") {
                  ...
                  let alertTitle = ChatroomState.type != "owner" ? "会诊已结束" : "抱歉，您被踢出(该账号在其他设备创建房间)"
                  Alert.open({
                    title: "提示",
                    msg:
  ...
  doGetChatroom() {
    this.getChatroom()
      .then(obj => {
        ...
        let remoteIsSharing = custom.shareID !== "" && custom.showType === 0;
        //设置远端是否正在屏幕共享
        NetcallAction.setHasShareScreen(remoteIsSharing);
        NetcallAction.setRemoteIsScreenSharing(remoteIsSharing);
        ChatroomAction.setCustom(custom)
      })
      .catch(error => {
  ...
  close(roomId) {
    ...
    const param = {
      url: env.url + "/api/netease/chatroom/close",
      type: "POST",
      data: {
        // uid: StoreNim.state.account,
        operator: StoreNim.state.account,
        roomid: roomId
      }
    };
    return Ajax(param)
  ...
  logout(chatroomId, msg) {
    ...
    // window.location.href="about:blank";// 关闭当前窗口
    // window.close();
