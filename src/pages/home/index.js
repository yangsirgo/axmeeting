...
import { Toast, Alert, Storage, Page, Valid, CheckBroswer } from "util";
import { StoreNim, StoreChatroom, StoreNetcall } from "store";

import DeviceDoctor from './DeviceDoctor'
...
export default class Home extends Component {
  state = {
    roomName: "",
    deviceDoctorVisited: false,
    deviceDoctor: false,
    roomId: "",
    isNetDetectStatus: "网络探测",
    startDect: false,
    netDetectStatus: "",
    baseImfo: "",
  ...
  startNetDetect = evnet => {
    if(platform.name==='Firefox'){
      Alert.open({
        title: "提示",
        msg:
          '<div class="u-logoutim"><i class="u-icon-tip"></i> Firefox浏览器当前暂不支持网络检测</div>',
        isHtml: true,
        btns: [
          {
            label: "确认",
  ...
  showDeviceDoctor = () => {
    this.setState({ deviceDoctor: true })
  }

  hideDeviceDoctor = () => {
    this.setState({
      deviceDoctorVisited: true,
      deviceDoctor: false
    })
  }
  ...
  renderHome() {
    ...
    return (<div className="m-home">
      <div className="net-detect" style={state.isNetDetect}>
      ...
      <div className="part part-1">
        <button className="is-net-detect"
          onClick={this.startNetDetect}
          disabled={state.startDect}
        >
          {state.isNetDetectStatus}
        </button>
        ...
        <div className="tip detect-device">
          {state.deviceDoctorVisited
            ? <p className="f-tac"><Button className="cube-btn" onClick={this.showDeviceDoctor}>再次检测</Button></p>
            : <p><Button className="cube-btn" onClick={this.showDeviceDoctor}>设备检测</Button>&nbsp;&nbsp;首次使用新设备建议先进行设备检测~</p>}
  ...
  renderDeviceDoctor = () => {
    ...
    return (
      <div className="m-home">
        <DeviceDoctor hide={this.hideDeviceDoctor}></DeviceDoctor>
      </div>
    )
  }
  render() {
