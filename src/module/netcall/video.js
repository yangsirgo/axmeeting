...
export default class extends Component {
    ...
    if (v) {
      EXT_NETCALL.startCamera()
        .then(() => {
          ...
          let isTeacher = (NetcallState.doms[0] == this.dom) ? 1 : 0;
            console.log("NetcallState.doms[0]", NetcallState.doms[0]);
          EXT_NETCALL.setVideoViewSize(isTeacher);
  ...
  toggleFullscreen() {
    ...
    console.log("### 放大", account, dom);
      console.info("20200221 video 152")
  ...
  render() {
    ...
    const isSharing = account == shareID && shareID !== "";
    return (
      <div className={classNames("m-netcall-video", className)}>
        <div className="video-name">{nick}</div>
        <div
          ...
          !NetcallState.videoIsFullScreen && (
            <div className="video-toolbar">
              <span
                className={classNames(
                  "fullscreen",
                  !NetcallState.videoIsFullScreen ? "fullscreen-open" : ""
                )}
