...
export default class {
  @observable
  state = {
    remoteIsScreenSharing: false, //远端是否在屏幕共享状态标记
    videoIsFullScreen: false, //本端是否大屏显示老师屏幕共享画面
    doms: [], //RTC dom
  ...
  @action
  setRemoteIsScreenSharing(remoteIsScreenSharing) {
    this.state.remoteIsScreenSharing = remoteIsScreenSharing;
    console.log(
      "store --> netcall --> setRemoteIsScreenSharing",
  ...
  @action
  setVideoIsFullScreen(videoIsFullScreen) {
    this.state.videoIsFullScreen = videoIsFullScreen;
    console.log("20200218：store --> netcall --> setVideoIsFullScreen", videoIsFullScreen);
  }
  /**
